...
#   brew install ffmpeg   # macOS, needed by pydub
#   export OPENAI_API_KEY="your_api_key"
#   python make_podcast_audio.py input_script.txt output_podcast.mp3
...
def main():
    ...
    if len(sys.argv) not in [3, 5]:
        print("Usage:")
        print("  python make_podcast_audio.py input_script.txt output_podcast.mp3")
        print("  python make_podcast_audio.py input_script.txt output_podcast.mp3 intro.mp3 outro.mp3")
        sys.exit(1)

    input_file = Path(sys.argv[1])
    output_file = Path(sys.argv[2])

    intro_file = Path(sys.argv[3]) if len(sys.argv) == 5 else None
    outro_file = Path(sys.argv[4]) if len(sys.argv) == 5 else None
    ...
    if intro_file:
        intro_audio = AudioSegment.from_file(intro_file).apply_gain(-6)  # Reduce volume of intro
